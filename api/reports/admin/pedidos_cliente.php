        ...
        if ($rowCliente = $cliente->readOne()) {
            ...
            if ($dataClientes = $cliente->reporteCliente()) {
                ...
                foreach ($dataClientes as $rowCliente) {
                    $pdf->cell(85, 10, $pdf->encodeString($rowCliente['nombre_producto']), 1, 0);
                    $pdf->cell(40, 10, $rowCliente['direccion_pedido'], 1, 0);
                    $pdf->cell(30, 10, $rowCliente['cantidad_pedidos'], 1, 0);
                    $pdf->cell(30, 10, $rowCliente['fecha_registro'], 1, 1);

                }
            } else {
                // Si no hay pedidos, se muestra un mensaje en el reporte.
                $pdf->cell(0, 10, $pdf->encodeString('No hay pedidos de este cliente'), 1, 1, 'C');
            }
