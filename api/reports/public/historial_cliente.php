...
if (isset($_GET['idPedido'])) {
    ...
    if ($historial->setId($_GET['idPedido'])) {
        ...
        if ($rowPedido = $historial->reporteHistorial()) {
            ...
            $pdf->Write(10, $pdf->encodeString("Nombre : {$item['nombre_cliente']}"));
            $pdf->Ln(10); // Salto de línea
            $pdf->Write(10, $pdf->encodeString("Apellido : {$item['apellido_cliente']}"));
            $pdf->Ln(10); // Salto de línea
            $pdf->Write(10, $pdf->encodeString("Correo electroníco : {$item['correo_cliente']}"));
            ...
            foreach ($rowPedido as $item) {
                $pdf->cell(90, 10, $pdf->encodeString($item['nombre_producto']), 1, 0);
                $pdf->cell(40, 10, $item['precio_pedido'], 1, 0);
                $pdf->cell(30, 10, $item['cantidad_pedido'], 1, 0);
                $pdf->cell(30, 10, $item['fecha_registro'], 1, 1);
            }
        } else {
