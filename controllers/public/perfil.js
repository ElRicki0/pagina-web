...
const PROFILE_FORM = document.getElementById('profileForm'),
    ...
    RESIDENCIA_CLIENTE = document.getElementById('direccionCliente'),
    ALIAS_CLIENTE = document.getElementById('aliasCliente');

const IMAGEN_I = document.getElementById('imagenCliente');
const IMAGEN_R = document.getElementById('imagenR');
...
IMAGEN_I.addEventListener('change', function (event) {
    ...
    if (event.target.files && event.target.files[0]) {
        ...
        reader.onload = function (event) {
            ...
            IMAGEN_R.src = event.target.result;
        };
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    MAIN_TITLE.textContent = 'Datos de cuenta';
